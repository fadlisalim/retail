...
<div
    ...
    x-data="csChat({ endpoint: '{{ route('assistant.chat') }}', history: '{{ route('assistant.history') }}', contact: '{{ route('assistant.contact') }}', click: '{{ route('assistant.click') }}', welcomes: @js($csWelcomes) })"
                        ...
                        <template x-if="m.role === 'assistant' && m.products && m.products.length">
                            <div class="space-y-2">
                                <template x-for="p in m.products" :key="p.url">
                                    <a :href="p.url" @click="track('product', p.slug)" class="flex items-center gap-2 rounded-xl border border-gray-200 bg-white p-2 shadow-sm transition hover:border-brand-300">
