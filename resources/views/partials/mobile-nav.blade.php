...
<nav class="fixed inset-x-0 bottom-0 z-40 border-t border-gray-200 bg-white lg:hidden" aria-label="Navigasi bawah">
    <div class="mx-auto grid max-w-lg grid-cols-5">
        @php($nav = [
            ...
            ['search', 'Cari', 'm21 21-4.35-4.35m1.35-5.4a6.75 6.75 0 1 1-13.5 0 6.75 6.75 0 0 1 13.5 0Z'],
        ])
        ...
        @endforeach
        <a href="{{ route('cart.index') }}" class="relative flex flex-col items-center gap-0.5 py-2 text-[11px] {{ request()->routeIs('cart.*') ? 'text-brand-700' : 'text-gray-500' }}">
            ...
            @if($cartCount)<span class="absolute right-6 top-1 grid h-4 min-w-4 place-items-center rounded-full bg-accent-500 px-1 text-[10px] font-bold text-white">{{ $cartCount }}</span>@endif
            Keranjang
        </a>
